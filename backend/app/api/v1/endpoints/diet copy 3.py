...
def get_model_input(img_path):
    # 1. Letterbox 리사이즈 (640x640)
    img = preprocess_yolo(img_path) 
    
    # 2. BGR -> RGB 변환 (OpenCV는 기본이 BGR임)
    img = img[:, :, ::-1] 
    
    # 3. HWC(높이,너비,채널) -> CHW(채널,높이,너비)로 순서 변경
    img = img.transpose(2, 0, 1)
    
    # 4. 0~1 사이로 정규화 및 배치 차원 추가
...
with torch.no_grad():
    ...
    # 여기서 신뢰도(conf)와 겹침 정도(iou) 임계값을 잘 조절해야 인식률이 좋아집니다.
    # 인식이 너무 안 되면 conf를 낮추고, 겹친 박스가 많으면 iou를 낮추세요.
...
DB_PATH = os.path.join(BASE_DIR, "data", "food_info2.csv") # 클래스인덱스
MODEL_PATH = os.path.join(BASE_DIR, "models", "food", "efficientnetb0.pt") # 음식인식
