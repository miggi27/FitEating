...
# 수정 코드 (영상 파일 경로 입력)
video_file_path = "./resources/videos/4921641-sd_356_640_25fps.mp4" # 본인의 영상 파일명으로 바꾸세요!
...
while True:
    ...
    # 영상을 다 읽었거나 파일을 못 찾으면 종료
    if not ret:
        st.warning("영상이 끝났거나 파일을 찾을 수 없습니다.")
        break
    ...
    try:
        if results_yolo is not None:
            ...
            if results_pose.pose_landmarks is not None:
                ...
                # 각도 계산 직후 (130~140라인 근처) 작동확인용!!!!
                # if left_knee_angle < 100: # 무릎이 어느 정도 굽혀지면 바로 소리 지르기
                #     st.write("오! 무릎이 굽혀지는 걸 감지했습니다!")
